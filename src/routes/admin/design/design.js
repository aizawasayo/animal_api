...
export default async (req, res) => {
    ...
    let count = null
    await Design.countDocuments(condition, (err, counts) => { //查询符合条件的总条数
        if (counts) {
            count = counts
        }

    })
    await Design.find(condition).skip(skip).limit(limit).populate('user').sort(sortCondition).collation({
        locale: 'zh'
    }).exec((err, list) => {
